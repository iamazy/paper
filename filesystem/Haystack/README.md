...
如第 3 节所述，生产环境下对 Store 的写入操作总是进行多写入操作，以分摊写入操作的固定成本。因为每张图片的 4 种不同尺寸都存储在 Haystack 中，因此找到图像组合相当简单。用户将一批图片上传到相册也很常见。由于这两个因素的结合，这个可写主机每个多写入操作平均写入 9.27 张图像。

第 4.1.2 节解释了最近上传的图片的读取和删除速率都很高，随着时间的推移而降低。这种行为也可以在图 10 中观察到；即使一些读取流量由缓存服务，可写入的盒子也会看到更多的请求。

值得注意的另一个趋势：随着越来越多的数据写入可写主机，图片的数量增加，导致读取请求速率的增加。

![图11](./img/figure11.png)

图11显示了与图10相同的两台机器在相同时期的读取和多写入操作的延迟。

多写入操作的延迟相当低（在1到2毫秒之间），即使流量的体积发生巨大变化也非常稳定。Haystack 机器具有支持 NVRAM 的 RAID 控制器，可以为我们缓冲写入操作。如第3节所述，NVRAM 允许我们异步写入 Needle，然后在多写入操作完成后发出单个 fsync 来刷新卷文件。多写入操作的延迟非常平稳和稳定。

只读主机上的读取延迟即使在流量显著变化的情况下（在3周内最高可达3倍）也相当稳定。对于可写主机，读取性能受到三个主要因素的影响。首先，随着存储在机器上的图片数量增加，对该机器的读取流量也会增加（请参见图10中的周对比流量）。其次，可写主机中的图片在缓存中缓存，而只读主机中的图片则不缓存3。这表明缓冲高速缓存对只读主机更有效。第三，最近写入的图片通常会立即被读取，因为 Facebook 强调最近的内容。在可写主机上进行此类读取操作将始终在缓冲高速缓存中命中，并提高缓冲高速缓存的命中率。图中的线条形状是这三个因素的结合结果。
